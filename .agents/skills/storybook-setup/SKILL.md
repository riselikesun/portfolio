...
description: >-
  Use this skill to configure Storybook with MSW and test configurations when the user asks to run npx storybook skills setup or configure Storybook testing.
---
...
| Config Dir | `/home/suraj/workspace/portfolio/.storybook` |
| Language | TypeScript |
| Package Manager | npm |
| Addons | @chromatic-com/storybook, @storybook/addon-vitest, @storybook/addon-a11y, @storybook/addon-docs, @storybook/addon-mcp |

Your goal is to make Storybook fully functional in this project: configure `/home/suraj/workspace/portfolio/.storybook/preview.tsx` with the right decorators, add MSW for data, and write up to 10 colocated `*.stories.tsx` files. Add `play` functions only where they prove something non-trivial.
...
4. **Edit > Write.** For any file you've Read, use `Edit`. Use `Write` only for new files. The project already has a `/home/suraj/workspace/portfolio/.storybook/preview.tsx` from `storybook init` — **Edit** it, do not overwrite.
...
6. **Use `npm` for installs and `npx storybook` for Storybook/Vitest CLI commands** (detected from this project's lockfile). Do not use `npx` — it invokes npm and fails when the repo enforces a different package manager.
7. **Prefer fixing the shared `/home/suraj/workspace/portfolio/.storybook/preview.tsx`** over story-local workarounds when multiple stories fail the same way.
...
### Step 2 — Build the shared preview
    ...
    Set up Storybook **once** so most stories work without per-story setup. **Edit the existing `/home/suraj/workspace/portfolio/.storybook/preview.tsx`** (created by `storybook init`) — add to its existing config object, don't replace it.

The complete shape should look like this (merge the new pieces into what's already there):

```tsx
// /home/suraj/workspace/portfolio/.storybook/preview.tsx
...
If you found `createPortal(..., document.getElementById('foo'))` in discovery, **add a decorator in `/home/suraj/workspace/portfolio/.storybook/preview.tsx` that creates the portal root** before the story renders. Do not use `preview-body.html`.
...
Use `msw-storybook-addon`. Register it with `storybook add` (which also adds it to the `addons` field of `/home/suraj/workspace/portfolio/.storybook/main.ts`), then install its peer dependencies and generate the worker script:
    ...
    Make sure `/home/suraj/workspace/portfolio/.storybook/main.ts` serves `./public`:

    ```ts
// /home/suraj/workspace/portfolio/.storybook/main.ts
...
```
    ...
    Put handlers in `/home/suraj/workspace/portfolio/.storybook/msw-handlers.ts`. Cover only the endpoints your stories will exercise — no catch-alls.

    ```ts
    // /home/suraj/workspace/portfolio/.storybook/msw-handlers.ts
